...
6. **Modelo de Datos Relacional (DER):** Diseño de base de datos altamente normalizado para soportar la complejidad del ERP. Utiliza estrategias de mapeo avanzadas (como *Single Table* para jerarquías de herencia) y tablas puente estratégicas (`Consumo`, `Movimientos`) que garantizan la trazabilidad absoluta del Kardex (desde la compra de la materia prima hasta la ejecución de la Orden de Trabajo).
